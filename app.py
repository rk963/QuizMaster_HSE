...
defaults = {
    "text": "",
    "file_bytes": None,       # NEW: raw bytes of uploaded file for chunk cache
    "file_name": None,        # NEW: filename for cache key display
    "quiz": [],
    "answers": {},
    "submitted": False,
    "is_generating": False,
...
def _start_generation(text: str, n_questions: int, language_code: str, file_bytes=None):
    ...
    def worker():
        try:
            # Pass file_bytes into state so RetrieverAgent can use the chunk cache
...
if uploaded is not None:
    try:
        ...
        # ---- OPTIMIZATION: skip re-extraction if same file is re-uploaded ----
        if st.session_state.file_name == uploaded.name and st.session_state.text:
    ...
    file_bytes = None  # pasted text has no cache key
...
# ---- OPTIMIZATION: faster poll interval (0.5s instead of 1.0s) ----
if st.session_state.is_generating:
    time.sleep(0.5)
    st.rerun()
